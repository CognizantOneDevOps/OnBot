...
module.exports = (robot) ->
	robot.respond /getHitmiss (.*)/i, (msg) ->
		...
		masterbot_hitmiss.masterbot_hitmiss first,(error, stdout, stderr) ->
			if stdout
				str += '(continue)' + ' HITMISS RATIO : ' + ((stdout.hitmiss/stdout.totalconv) * 100).toFixed() + '%\n' # Calculating HitMiss Ratio
				str += '(continue)' + ' TOTAL_CONVERSATIONS : ' + stdout.totalconv # total conversations with the bot
				msg.send str;
			else if stderr
				msg.send stderr;
			else if error
