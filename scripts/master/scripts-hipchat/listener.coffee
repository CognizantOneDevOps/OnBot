...
module.exports = (robot) ->
	...
	robot.respond /addBot (.*) (.*) (.*)/i, (msg) ->
		...
		jidinitial = msg.match[3].split("_")[0]
		jid = msg.match[3].split("_")[1].split("@")[0]
		msg.send "Bot is getting Deployed....\n You will be notified when deployment is done"
		addbots.addbots fileid,filename,jidinitial,jid,(error, stdout, stderr) ->
	...
	robot.respond /getConfig Template (.*)/i, (msg) ->
		bottype = msg.match[1]
		getdetailbot.getdetailbot bottype,(error, stdout, stderr) ->
			if(stdout)
				msg.send(stdout)
			if(stderr)
				msg.send(stderr)
