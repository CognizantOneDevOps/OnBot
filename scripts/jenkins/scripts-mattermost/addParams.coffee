...
module.exports = (robot) ->
	cmd_give_config=new RegExp('@' + process.env.HUBOT_NAME + ' give (.*) config')
	robot.listen(
		(message) ->
			return unless message.text
			message.text.match cmd_give_config
		(msg) ->
			console.log "inside addParams"
			jobname=msg.match[1]
	...
	cmd_upload_config=new RegExp('@' + process.env.HUBOT_NAME + ' upload (.*) config')
	robot.listen(
		(message) ->
			return unless message.text
			message.text.match cmd_upload_config
		(msg) ->
			jobname=msg.match[1]
			url=jenkins_url+"/job/"+jobname+"/config.xml"
