	...
	robot.respond /assign jira issue (.*) to (.*)/i, (msg) ->
		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			if stdout.jiraassignissue.workflowflag == true
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"jiraassignissue",assignee:assignee,Jir_ticket:Jir_ticket}
					message = {"text": "Request from "+user+" to assign jira issue of project "+Jir_ticket+" to "+assignee,"attachments": [{"text": "You can Approve or Reject","fallback": "failed to respond","callback_id": "jiraassignissue","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approve","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /edit jira issue (.*) with description (.*) and comment (.*)/i, (msg) ->
		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			if stdout.jiraeditissue.workflowflag == true
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"jiraeditissue",Jir_ticket:Jir_ticket,description:description,comment:comment}
					message = {"text": "Request from "+user+" to edit jira issue of project "+Jir_ticket,"attachments": [{"text": "You can Approve or Reject","fallback": "failed to respond","callback_id": "jiraeditissue","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approve","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /add comment (.*) to jira issue (.*)/i, (res) ->
		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			if stdout.jiraaddcomment.workflowflag == true
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:res.message.user.name,userid:res.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"jiraaddcomment",Jir_ticket:Jir_ticket,comment:comment}
					message = {"text": "Request from "+user+" to add comment to jira issue "+Jir_ticket,"attachments": [{"text": "You can Approve or Reject","fallback": "failed to respond","callback_id": "jiraaddcomment","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approve","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /update summary of issue (.*) as (.*)/i, (msg) ->
		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			if stdout.jiraupdatesummary.workflowflag == true
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"jiraupdatesummary",Jir_ticket:Jir_ticket,summary:summary}
					message = {"text": "Request from "+user+" to update summary of jira issue "+Jir_ticket,"attachments": [{"text": "You can Approve or Reject","fallback": "failed to respond","callback_id": "jiraupdatesummary","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approve","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /change status of issue (.*) to (.*)/i, (msg) ->
		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			if stdout.jirachangestatus.workflowflag == true
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"jirachangestatus",Jir_ticket:Jir_ticket,status:status}
					message = {"text": "Request from "+user+" to change status of jira issue "+Jir_ticket,"attachments": [{"text": "You can Approve or Reject","fallback": "failed to respond","callback_id": "jirachangestatus","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approve","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
