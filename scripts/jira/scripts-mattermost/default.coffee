...
module.exports = (robot) ->
  robot.hear /.+/, (msg) ->
    commands = ["reload","show watchers","search for","change status of issue","use filter","show filter","create jira issue","assign jira issue","close jira issue","edit jira issue","add comment","update summary","upcoming status","save filter","help"]
    message = msg.message
    message.text = message.text or ''
    regex = /([^\w\-]|^)(\w+-[0-9]+)(?=[^\w]|$)/
    result = regex.exec(message.text)
