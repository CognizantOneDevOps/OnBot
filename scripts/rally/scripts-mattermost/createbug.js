...
var createbug = function (url, username, password, name, desc, priority, severity, state, callback){
var bugbody={
  "Defect": {
    "Name": name,
    "Description": desc,
...
request(options, function (error, response, body) {
			...
			if (body){
			var message="DefectName \t\t\t DefectRef \t\t\t DefectId \t\t\t WorkspaceName \t\t\t WorkspaceId \n"
			if (response.statusCode==200)
			{
				if (body.CreateResult.Errors[0]!=undefined){
					...
					body.CreateResult.Object.Workspace._ref.split('/')[7]
