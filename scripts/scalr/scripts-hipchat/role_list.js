...
var function_call = function (main_scalr_url, access_id, access_key, envid, farm_id, callback_role_list) {
var path = '/api/v1beta0/user/' + envid + '/farms/'+farm_id+'/farm-roles/';
...
var options = { method: 'GET',
	...
	request(options, function (error, response, body) {
		...
		{
			callback_role_list("Wrong input","Wrong input","Wrong input");
		}
		else
		{
		...
		{
			var z = i+1;
			final_str = final_str + z +'.'+ " Name: " + body.data[i].alias + " ID: " +body.data[i].id + " FarmID: "+body.data[i].farm.id +"\n";
		}
		

			if (error)
			{
				callback_role_list("Something went wrong","Something went wrong","Something went wrong");
				
		}		
			else{
				
			callback_role_list(null,final_str,null);
...
module.exports = {
 role_list: function_call	// MAIN FUNCTION
