...
var function_call = function (main_scalr_url, access_id, access_key, envid, farm_id, callback_server_list) {
var farmid = farm_id;
...
var options = { method: 'GET',
	...
	request(options, function (error, response, body) {
		...
		{
			var z = i+1;
			final_str = final_str + z + " Type: " + body.data[i].type + " ID: " +body.data[i].id + " CloudID: "+body.data[i].cloudServerId + " CloudPlatfrom: "+body.data[i].cloudPlatform + " Status: "+body.data[i].status + " CloudLocation: "+body.data[i].cloudLocation+"\n";
		}
		

			if (error)
			{
				callback_server_list("Something went wrong","Something went wrong","Something went wrong");
				
		}		
			else{
				
			callback_server_list(null,final_str,null);
...
module.exports = {
 server_list: function_call	// MAIN FUNCTION
