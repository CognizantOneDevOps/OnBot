		...
		#Action Flow with workflow flag
			if(stdout.assigntask.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					...
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_ASSIGNTASK","taskid":taskid,"username":username}
					message='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: for assigning xlrelease task: '+taskid+' to '+username+'\n approve or reject the request'
		...
		#Action Flow with workflow flag
			if(stdout.comlpetetask.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for Hipchat	
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_COMPLETETASK","taskid":taskid,"comment":comment}
					message='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: for completing task '+taskid+'\n approve or reject the request'
		...
		#Action Flow with workflow flag
			if(stdout.taskstart.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for Hipchat	
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_STARTTASK","filename":filename,"taskid":taskid}
					message='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: starting task '+taskid+'\n approve or reject the request'
		...
		#Action Flow with workflow flag
			if(stdout.deltask.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for Hipchat	
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_DELTASK","taskid":taskid}
					message='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: for deleting task '+taskid+'\n approve or reject the request'
		...
		#Action Flow with workflow flag
			if(stdout.delrelease.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for Hipchat	
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_DELRELEASE","releaseid":releaseid}
					message='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: deleting release '+releaseid+'\n approve or reject the request'
		...
		#Action Flow with workflow flag
			if(stdout.deltemplate.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for Hipchat	
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_DELTEMPLATE","templateid":templateid}
					message='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: deleting template '+templateid+'\n approve or reject the request'
	...
	robot.router.post '/XLRELEASE_DELTEMPLATE', (request,response) ->
		...
		if data.action=='Approved'
			...
			deltemplate.templatedel xlrelease_url, username, password, data.templateid, (error, stdout, stderr) ->
				...
				if stdout
					setTimeout (->eindex.passData stdout),1000
					message = "delete template "+data.id
					actionmsg = "xlrelease template deleted"
					statusmsg = "success"
					eindex.wallData botname, message, actionmsg, statusmsg;
					console.log(stdout);
		...
		#Action Flow with workflow flag
			if(stdout.getbytiltle.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for Hipchat	
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_GETBYTITLE","name":name}
					message='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: for getting detail of '+name+'\n approve or reject the request'
