		...
		#Action Flow with workflow flag
			if(stdout.release.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					...
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_RELEASE","filename":filename,"templateid":templateid}
					message = {"text": "Request from "+msg.message.user.name+" for creating release from template"+templateid,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "XLRELEASE_RELEASE","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
		...
		#Action Flow with workflow flag
			if(stdout.phase.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for slack
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_PHASE","filename":filename,"templateid":templateid}
					message = {"text": "Request from "+msg.message.user.name+" for creating phase in "+templateid,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "XLRELEASE_PHASE","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
		...
		#Action Flow with workflow flag
			if(stdout.task.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for slack
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_TASK","filename":filename,"phaseid":phaseid}
					message = {"text": "Request from "+msg.message.user.name+" for creating task ","attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "XLRELEASE_TASK","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
		...
		#Action Flow with workflow flag
			if(stdout.template.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for slack
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_TEMPLATE","filename":filename}
					message = {"text": "Request from "+msg.message.user.name+" for creating template ","attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "XLRELEASE_TEMPLATE","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
		...
		#Action Flow with workflow flag
			if(stdout.releasestart.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for slack
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_START","releaseid":releaseid}
					message = {"text": "Request from "+msg.message.user.name+" for starting release "+releaseid,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "XLRELEASE_START","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
		...
		#Action Flow with workflow flag
			if(stdout.commenttask.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					#Prepare payload for slack
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"XLRELEASE_COMMENT","taskid":taskid,"comment":comment}
					message = {"text": "Request from "+msg.message.user.name+" for adding comment in "+taskid,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "XLRELEASE_COMMENT","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
