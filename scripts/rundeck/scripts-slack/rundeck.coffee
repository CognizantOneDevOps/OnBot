		...
		#Action Flow with workflow flag
			if(stdout.listjob.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"rundeck_list_job","projname":projname}
					message = {"text": "Request from "+msg.message.user.name+" for listjob "+projname,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "rundeck_list_job","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /list projects/i, (msg) ->
		...
		#Action Flow with workflow flag
			if(stdout.listproject.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"rundeck_list_project"}
					message = {"text": "Request from "+msg.message.user.name+" for listproject","attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "rundeck_list_project","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /create project (.*)/i, (msg) ->
		...
		#Action Flow with workflow flag
			if(stdout.createproject.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"rundeck_create_project","filename":filename}
					message = {"text": "Request from "+msg.message.user.name+" for create project ","attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "rundeck_create_project","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /get config (.*)/i, (msg) ->
		...
		#Action Flow with workflow flag
			if(stdout.getconfig.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"rundeck_get_config","projectname":projectname}
					message = {"text": "Request from "+msg.message.user.name+" for get config of "+projectname,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "rundeck_get_config","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /run job (.*)/i, (msg) ->
		...
		#Action Flow with workflow flag
			if(stdout.runjob.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"rundeck_run_job","jobid":jobid}
					message = {"text": "Request from "+msg.message.user.name+" for run job "+jobid,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "rundeck_run_job","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /delete job (.*)/i, (msg) ->
		...
		#Action Flow with workflow flag
			if(stdout.deletejob.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"rundeck_delete_job","jobid":jobid}
					message = {"text": "Request from "+msg.message.user.name+" for delete job "+jobid,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "rundeck_delete_job","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /delete project (.*)/i, (msg) ->
		...
		#Action Flow with workflow flag
			if(stdout.deleteproject.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"rundeck_delete_project","projectname":projectname}
					message = {"text": "Request from "+msg.message.user.name+" for delete project "+projectname,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "rundeck_delete_project","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
	...
	robot.respond /get history (.*)/i, (msg) ->
		...
		#Action Flow with workflow flag
			if(stdout.projecthistory.workflowflag)
				...
				generate_id.getNextSequence (err,id) ->
					tckid=id
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.id,podIp:process.env.MY_POD_IP,"callback_id":"rundeck_project_history","projectname":projectname}
					message = {"text": "Request from "+msg.message.user.name+" for get project history "+projectname,"attachments": [{"text": "U can Approve or Reject","fallback": "failed to respond","callback_id": "rundeck_project_history","color":"#3AA3E3","attachment_type":"default","actions":[{"name":"Approved","text":"Approve","type":"button","value":tckid},{"name":"Rejected","text": "Reject","type": "button","value": tckid,"style": "danger",confirm: {'title': 'Are you sure?','text': 'Do you want to Reject?','ok_text': 'Reject','dismiss_text': 'No'}}]}]}
