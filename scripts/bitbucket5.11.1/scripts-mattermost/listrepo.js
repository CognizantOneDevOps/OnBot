...
request(options, function (error, response, body) {
  ...
  if (response.statusCode==200){
	...
	for(i=0;i<body.values.length;i++){
		data+='|'+body.values[i].slug+'|'+body.values[i].id+'|'+body.values[i].name+'|'+'[go to project]('+body.values[i].project.links.self[0].href+')'+'|'+'[clone here]('+body.values[i].links.self[0].href+')'+'|\n'
		console.log(data)
