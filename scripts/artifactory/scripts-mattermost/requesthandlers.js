...
var getrepos = function (callback) {
	...
	request.get(options, function(error, response, body){
		if(error)
			...
			var reply = '|RowNum|Repo Type|Repo Key|\n|:------:|:------:|:------:|\n'
			...
			if(body.hasOwnProperty('errors'))
				...
				if(body.results.length == 0)
					...
					for(var i=0;i<body.results.length;i++){
						reply += ':arrow_down: [Download ' + body.results[i].uri.split('/')[body.results[i].uri.split('/').length-1] + '](' + process.env.ARTIFACTORY_HOST + '/artifactory/' + body.results[i].uri.substring(body.results[i].uri.indexOf(repo+'/'),body.results[i].uri.length) + ')\nRemote Path: '+body.results[i].uri.substring(body.results[i].uri.indexOf(repo+'/'),body.results[i].uri.length)+'\n'
					}
					callback(null,reply)
				}
			}
		...
		if(error)
			...
			if(body.hasOwnProperty('errors'))
				callback(null,body.errors[0].message)
					...
					var reply = '|RowNum|Username|Realm|\n|:----:|:----:|:----:|\n'
					for(var i=0;i<body.length;i++){
						reply += '|' + (i+1) + '|' + body[i].name + '|' + body[i].realm + '|\n'
					}
					callback(null,reply)
				}
			}
