		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			...
			if stdout.create_repo.workflowflag
				generate_id.getNextSequence (err,id) ->
					tckid=id
					console.log(tckid);
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"githubrepo","reponame":reponame}
		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			...
			if stdout.create_repo.workflowflag
				generate_id.getNextSequence (err,id) ->
					tckid=id
					console.log(tckid);
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"githuborgrepo","reponame":reponame,orgname:orgname}
					data='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: create repo '+reponame+' inside org:'+orgname+'\n approve or reject the request'
					robot.messageRoom(stdout.create_repo.adminid, data);
					msg.send 'Your request is waiting for approval by '+stdout.create_repo.admin
					dataToInsert = {ticketid: tckid, payload: payload, "status":"","approvedby":""}
					#Insert into Mongo with Payload
		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			...
			if stdout.delete_repo.workflowflag
				generate_id.getNextSequence (err,id) ->
					tckid=id
					console.log(tckid);
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"deletegithubrepo","reponame":reponame}
