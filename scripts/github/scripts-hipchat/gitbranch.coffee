		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			...
			if stdout.create_branch.workflowflag
				generate_id.getNextSequence (err,id) ->
					tckid=id
					console.log(tckid);
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"creategithubbranch",newbranch:newbranch,reponame:reponame,oldbranch:oldbranch}
					data='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: create branch '+newbranch+' inside repo:'+reponame+' from existing branch:'+oldbranch+'\n approve or reject the request'
		...
		readjson.readworkflow_coffee (error,stdout,stderr) ->
			...
			if stdout.delete_branch.workflowflag
				generate_id.getNextSequence (err,id) ->
					tckid=id
					console.log(tckid);
					payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.user.reply_to,podIp:process.env.MY_POD_IP,"callback_id":"deletegithubbranch",branchname:branchname,reponame:reponame}
					data='Ticket Id : '+tckid+'\n Raised By: '+msg.message.user.name+'\n Command: delete branch '+branchname+' inside repo:'+reponame+'\n approve or reject the request'
