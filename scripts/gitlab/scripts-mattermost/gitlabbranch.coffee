...
module.exports = (robot) ->
	cmdcreate = new RegExp('@' + process.env.HUBOT_NAME + ' create branch (.*) for (.*) from (.*)')
	robot.listen(
		(message) ->
			...
			readjson.readworkflow_coffee (error,stdout,stderr) ->
				...
				if stdout.gitlabcreatebranch.workflowflag
					generate_id.getNextSequence (err,id) ->
						tckid=id
						payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.room,approver:stdout.gitlabcreatebranch.admin,podIp:process.env.MY_POD_IP,message:msg.message.text,projectid:projectid,callback_id: 'gitlabcreatebranch',tckid:tckid,newbranch:newbranch,oldbranch:oldbranch};
						data = {"channel": stdout.gitlabcreatebranch.admin,"text":"Request from "+payload.username + " for creating a branch with ID: "+payload.projectid,"message":"Request from "+payload.username + " to create a branch for a project with ID: "+payload.projectid,attachments: [{text: 'click approve or reject',fallback: 'Yes or No?',callback_id: 'gitlabcreatebranch',color: '#3AA3E3',attachment_type: 'default',actions: [{ name: 'Approve', text: 'Approve', type: 'button',"integration": {"url": process.env.APPROVAL_APP_URL,"context": {"action": "Approve",value: tckid}}},{ name: 'Reject', text: 'Reject', type: 'button', "integration": {"url": process.env.APPROVAL_APP_URL,"context": {"action": "Reject",value: tckid}}}]}]}
						options = {
						...
						request.post options, (err,response,body) ->
							...
							msg.send 'Your request is waiting for approval from '+stdout.gitlabcreatebranch.admin
	...
	cmddelete = new RegExp('@' + process.env.HUBOT_NAME + ' delete branch (.*) from (.*)')
	robot.listen(
		(message) ->
			return unless message.text
		...
		(msg) ->
			...
			readjson.readworkflow_coffee (error,stdout,stderr) ->
				...
				if stdout.gitlabdeletebranch.workflowflag
					generate_id.getNextSequence (err,id) ->
						tckid=id
						payload={botname:process.env.HUBOT_NAME,username:msg.message.user.name,userid:msg.message.room,approver:stdout.gitlabdeletebranch.admin,podIp:process.env.MY_POD_IP,message:msg.message.text,projectid:projectid,callback_id: 'gitlabdeletebranch',tckid:tckid,branch:branch};
						data = {"channel": stdout.gitlabdeletebranch.admin,"text":"Request from "+payload.username + " for deleting branch from project with ID: "+payload.projectid,"message":"Request from "+payload.username + " to delete branch from project with ID: "+payload.projectid,attachments: [{text: 'click approve or reject',fallback: 'Yes or No?',callback_id: 'gitlabdeletebranch',color: '#3AA3E3',attachment_type: 'default',actions: [{ name: 'Approve', text: 'Approve', type: 'button',"integration": {"url":  process.env.APPROVAL_APP_URL,"context": {"action": "Approve",value: tckid}}},{ name: 'Reject', text: 'Reject', type: 'button', "integration": {"url":  process.env.APPROVAL_APP_URL,"context": {"action": "Reject",value: tckid}}}]}]}
						options = {
							url: process.env.MATTERMOST_INCOME_URL,
							method: "POST",
							header: {"Content-type":"application/json"},
						...
						request.post options, (err,response,body) ->
							console.log response.body
							msg.send 'Your request is waiting for approval from '+stdout.gitlabdeletebranch.admin
						dataToInsert = {ticketid: tckid, payload: payload, "status":"","approvedby":""}
						generate_id.add_in_mongo dataToInsert
				else
					branch=msg.match[1]
